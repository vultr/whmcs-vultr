...
add_hook("ClientAreaFooterOutput", 1, function ($params){
	...
	if (isset($_GET['a']) && $_GET['a'] === 'confproduct')
	{
		require_once 'Loader.php';
		new main\Loader();
		$productInfo = WHMCS\Database\Capsule::table("tblproducts")->select("tblproducts.configoption2", "tblproducts.servertype")
			->where("tblproducts.id", "=", $_SESSION['cart']['products'][$_GET['i']]['pid'])->first();
		...
		$apiToken = \WHMCS\Database\Capsule::table("tbladdonmodules")->select("tbladdonmodules.value")
			->where("tbladdonmodules.module", "=", "vultr")
			->where("tbladdonmodules.setting", "=", "apiToken")->first();

...
<div class="col-sm-{$colWidth}">
	<input name="{$nameAttr}" {if $id} id="{$id}" {elseif $addIDs}id="{$addIDs}_{$name}"{/if} type="password"
		   value="{$value}"
		   class="form-control" {foreach from=$dataAttr key=dataKey item=dataValue}data-{$dataKey}="{$dataValue}"{/foreach} {if $addIDs}id="{$addIDs}_{$name}"{/if}
		   placeholder="{if $enablePlaceholder}{$MGLANG->T('placeholder')}{/if}" {if $required}required{/if}>

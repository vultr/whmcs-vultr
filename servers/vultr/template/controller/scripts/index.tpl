        ...
        <div class="panel-heading">
            <div class="row">
                ...
                <div class="col-sm-offset-2 col-sm-2">
                    <a class="btn btn-success" type="button" href="{$WEB_ROOT}/clientarea.php?action=productdetails&id={$serviceid}&cloudController=Scripts&cloudAction=add">{$_LANG.scripts.index.add}</a>
                </div>
            </div>            
        </div>
        <table class="table">
            ...
            {foreach from=$scripts item=script}
                <tr>
                    ...
                    <td><button scriptID="{$script['SCRIPTID']}" type="button" class="btn btn-info showScript">{$_LANG.scripts.index.show}</button> <a onclick="return confirm('{$_LANG.scripts.index.confirm}')" class="btn btn-danger" type="button" href="{$WEB_ROOT}/clientarea.php?action=productdetails&id={$serviceid}&cloudController=Scripts&cloudAction=delete&vultrID={$script['SCRIPTID']}">{$_LANG.scripts.index.delete}</a></td>
                </tr>
                <tr class="hide" scriptID='{$script['SCRIPTID']}'>
                    <td colspan="5"><div class="col-sm-12 text-left"><pre>{$script['script']}</pre></div></td>
                </tr>
            {foreachelse}
                <tr>
                    <td colspan="5">

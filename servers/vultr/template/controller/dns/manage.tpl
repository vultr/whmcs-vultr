...
<div class="row" id="vultrDNSContainer">
    <div class="panel panel-default col-sm-12">
        <form action="{$WEB_ROOT}/clientarea.php?action=productdetails&id={$serviceid}&cloudController=Dns&cloudAction=manage&vultrID={$domain}" method="POST">
            ...
            <table class="table table-striped table-bordered">
                ...
                {foreach from=$records item=record}
                    <tr>
                        ...
                        <td>{$record['type']}</td>
                        <td><input style="width: 190px;" name="record[{$record['RECORDID']}][name]" value="{$record['name']|escape:html}"></td>
                        <td><input style="width: 190px;" name="record[{$record['RECORDID']}][data]" value="{$record['data']|escape:html}"></td>
                        <td><input style="width: 50px;" name="record[{$record['RECORDID']}][ttl]" value="{$record['ttl']}"></td>
                        <td>{if $record['type']=='MX' || $record['type']=='SRV'}<input style="width: 50px;" name="record[{$record['RECORDID']}][priority]" value="{$record['priority']|escape:html}">{/if}</td>
                    </tr>
                {foreachelse}
                <tr>
                    <td colspan="5">
        ...
        <form action="{$WEB_ROOT}/clientarea.php?action=productdetails&id={$serviceid}&cloudController=Dns&cloudAction=manage&vultrID={$domain}" method="POST">
            ...
            <table class="table table-striped table-bordered">
                ...
                <tr class="addRecord">
                    ...
                    <td>
                        <button style="width: 50px;" type="submit" name="vultrAddAction" class="btn btn-default">{$_LANG.dns.manage.add}</button>
                    </td>            
                </tr>
            </table>
        </form>

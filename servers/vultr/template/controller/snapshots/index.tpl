        ...
        <div class="panel-heading">
            <div class="row">
                ...
                <div class="col-sm-offset-2 col-sm-2">
                    {if $use<$available}
                    <a class="btn btn-success" type="button" href="{$WEB_ROOT}/clientarea.php?action=productdetails&id={$serviceid}&cloudController=Snapshots&cloudAction=add">{$_LANG['snapshots']['index']['create_new']}</a>
                    {/if}
                </div>
            </div>            
        </div>
            ...
            {foreach from=$snapshots item=script}
                <tr>
                    ...
                    <td>
                        ...
                        {else}
                            <a onclick="return confirm('{$_LANG.snapshots.index.confirmrestore}')" class="btn btn-info" type="button" href="{$WEB_ROOT}/clientarea.php?action=productdetails&id={$serviceid}&cloudController=Snapshots&cloudAction=restore&vultrID={$script['SNAPSHOTID']}">{$_LANG.snapshots.index.restore}</a>
                            <a onclick="return confirm('{$_LANG.snapshots.index.confirm}')" class="btn btn-danger" type="button" href="{$WEB_ROOT}/clientarea.php?action=productdetails&id={$serviceid}&cloudController=Snapshots&cloudAction=delete&vultrID={$script['SNAPSHOTID']}">{$_LANG.snapshots.index.delete}</a></td>
                        {/if}
                </tr>
            {foreachelse}
                <tr>

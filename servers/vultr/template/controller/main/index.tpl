...
{if isset($server.power_status)}
    <div class="row" id="vultrMAINContainer">
        ...
        <div class="col-sm-12">
            ...
            <div id="vultrAjaxLoader" style="background: rgba(255, 255, 255, 0.8) url('{$WEB_ROOT}/modules/servers/vultr/assets/images/loader.gif') no-repeat scroll 50% 50%; display: none;">

            </div>
            <div class="so_buttons">
                <button class="btn" id="btn_boot">
                    <img class="manage_img" src="{$WEB_ROOT}/modules/servers/vultr/assets/images/reboot.png" /> {$_LANG.main.index.boot}
                </button>
                <button class="btn" id="btn_reboot">
                    <img class="manage_img" src="{$WEB_ROOT}/modules/servers/vultr/assets/images/repeat.png" /> {$_LANG.main.index.reboot}
                </button>
                <button class="btn" id="btn_stop">
                    <img class="manage_img" src="{$WEB_ROOT}/modules/servers/vultr/assets/images/stop.png" /> {$_LANG.main.index.stop}
                </button>
                <button class="btn" id="btn_reinstall">
                    <img class="manage_img" src="{$WEB_ROOT}/modules/servers/vultr/assets/images/package.png" /> {$_LANG.main.index.reinstall}
                </button>  
                <button class="btn" id="btn_console" onclick="window.open('{$server.kvm_url}', '{addslashes($server.label)}', 'directories=no,titlebar=no,toolbar=no,location=no,status=no,menubar=no,width=720,height=435');
                        return false;">
            ...
            <table class="table table-bordered table-striped" style="width:100%;">        
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <a id="checkStatus" href="">
                            <span id="serverstatus" style="display: none;"><img src="{$WEB_ROOT}/modules/servers/vultr/assets/images/ajax-loader.gif"></span>
                            <img id="refresh-loader" src="{$WEB_ROOT}/modules/servers/vultr/assets/images/refresh.gif" style="display: inline-block;" class="">
                        </a>                     
                    </td>
                </tr>
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <form id="vultr_label_form" class="form-horizontal hide" role="form" action="{$WEB_ROOT}/clientarea.php?action=productdetails&id={$serviceid}" method="POST">
                            <input class="form-control" maxlength="250" type="text" value="{$server.label}" name="vultrLabel"/>        
                            <button type="submit" name="vultrLabelAction" class="btn btn-primary">{$_LANG.main.index.save}</button>
                ...
                <div class="tab-content">
                        ...
                        <table class="table table-bordered table-striped" style="width:100%;">
                            ...
                            {foreach from=$ipv4 item=ip}        
                                <tr>
                                    ...
                                    <td>
                                        {if $ip.type!='private'}
                                            <form id="rev_{$ip.ip|replace:'.':'_'}" class="hide" action="{$WEB_ROOT}/clientarea.php?action=productdetails&id={$serviceid}" method="POST">
                                                <input type="hidden" value="{$ip.ip}" name="vultrIP">
                                                <input value="{$ip.reverse}" name="vultrREVDNS" style="margin-bottom: 5px;"/>
                                                <button class="btn btn-primary" type="submit">Update</button>
                                                <button class="btn" onclick="$('form#rev_{$ip.ip|replace:'.':'_'}').addClass('hide');
                    ...
                    {if !empty($ipv6)}
                        <div role="tabpanel" class="tab-pane" id="ipv6tab">
                            ...
                            <form class="form-horizontal" role="form" action="{$WEB_ROOT}/clientarea.php?action=productdetails&id={$serviceid}" method="POST">
                                <table class="table table-bordered table-striped" style="width:100%;">
                                    <tr>
                                    ...
                                    {foreach from=$ipv6rev item=ip}        
                                        <tr>
                                            ...
                                            <td>{$ip.reverse}</td>
                                            <td>
                                                <button attr-ip="{$ip.ip}" attr-rev="{$ip.reverse}" style="margin-bottom: 5px;" class="btn btn-primary edit-rev-dns" id="{$ip.ip|replace:':':''}">{$_LANG.main.index.edit}</button>
                                                <a onclick="return confirm('{$_LANG.main.index.confirm}')" class="btn btn-danger" type="button" href="{$WEB_ROOT}/clientarea.php?action=productdetails&id={$serviceid}&cloudController=Main&cloudAction=delete&vultrID={$ip.ip}">{$_LANG.main.index.delete}</a>
                                            </td>
                                        </tr>
                                    {/foreach}
                                    <tr>
                                        ...
                                        <td><button type="submit" name="ipv6revAdd" class="btn btn-primary">{$_LANG.main.index.add}</button></td>
                                    </tr>
                                </table>
                            </form>
                        </div>
        ...
        <div class="col-sm-12">
            <table class="table table-bordered table-striped" style="width:100%;">
                <tr>
                    ...
                    <td class="col-sm-10">
                        <div class="progress">
                            <div class="progress-bar" role="progressbar" aria-valuenow="{$server.bandwidth}" aria-valuemin="0" aria-valuemax="100" style="min-width: 0em; width:{$server.bandwidth}%;">
                                <span class="progress-value" style="position:absolute;right:0;left:0;font-size: 16px;color: #333333;">{$server.current_bandwidth_gb} GB / {$server.allowed_bandwidth_gb} GB</span>
                            </div>
                        </div>
                    </td>
                </tr> 
                <tr>
                    <td colspan="2"><a href="clientarea.php?action=productdetails&id={$module['serviceid']}&cloudController=Graphs" class="btn btn-default" type="button">{$_LANG.main.index.usage_graphs}</a></td>
                </tr>
            </table>
        </div>
    </div>
    {literal}
        <style type="text/css">
            .so_buttons {margin-bottom:20px;}
            .so_buttons a, .so_buttons form {display:block; float:left;overflow:hidden; margin:3px; }
            .so_title {margin-bottom:20px;}

            .so_progress {width:58%;height:20px;}
            .so_progress-wrap {background:green;overflow:hidden;position:relative;float:left;line-height:20px;}
            .so_progress-bar {background: #ddd;position: absolute;top: 0;width:100%;padding-left:5px;}
            .so_progress-total {float: right; width:40%; line-height:20px;}

            #so_root_pass, #so_hostname {margin:0;padding:0;}
            #so_root, #so_root_pass, #so_root_change_act, #so_hostname, #btn_change_hostname {display:none;}

            #so_alerts .box-success, #so_alerts .box-error {padding:5px;margin-bottom:20px;}
            #so_alerts .box-success {background-color: #DFF0D8;}
            #so_alerts .box-error {background-color: #F2DEDE;}

            .bs-docs-example:after {background-color: #F5F5F5;border: 1px solid #DDDDDD;border-radius: 4px 0 4px 0;color: #9DA0A4;font-size: 12px;font-weight: bold;left: -1px;padding: 3px 7px;position: absolute;top: -1px;}
            .bs-docs-example {background-color: #FFFFFF;border: 1px solid #DDDDDD;border-radius: 4px 4px 4px 4px;margin: 15px 0;padding: 39px 19px 14px;position: relative;}
            .bs-docs-example img {width:700px; margin: 0 auto;display:block;}
            .so_buttons {margin-bottom:20px; text-align: left}
            .so_buttons button {width:24%;margin-bottom: 5px;}

$(document).ready(function () {
    ...
    $('form[name="packagefrm"] div.tab-content div#tab3').on('click', 'a#vultr_configurable_options', function (e) {
        $.post("configproducts.php?action=edit&id=#id#&tab=3", {productID: '#id#', vultr_action: 'vultr_configurable_options'}, function (data) {
            alert(data.message);
    ...
    $('form[name="packagefrm"] div.tab-content div#tab3').on('click', 'a#vultr_custom_fields', function (e) {
        $.post("configproducts.php?action=edit&id=#id#&tab=3", {productID: '#id#', vultr_action: 'vultr_custom_fields'}, function (data) {
            alert(data.message);
            if (data.reload !== undefined) {
                location.reload();
            }
